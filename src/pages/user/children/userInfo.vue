<template>
  <div class="container">
    <div class="top flex" ref="top">
      <search @getSearch="getSearch" :title="'请输入校园号或昵称'"></search>
    </div>
    <scrollContentBox :topClientHeight="topHeight" @ScrollBottom="loadImg">
      <div>
        <user :user="item" v-for="item in users"></user>
...
import { getFindTopic, getAllUser } from "@/request/api"
import { ResultData,AuthorInfo } from '@/types/interfaces'
...
// 保存话题数组
let users = reactive<Array<AuthorInfo>>([]);
...
type Methods = 'search' | 'onload' | never
